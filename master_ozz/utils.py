...
def init_user_session_state(prod, db_root, init_sessstate=True):
    if pg_migration:
...
def save_json(db_name, data, log=True):
    # if pg_migration:
    #     up = PollenDatabase.upsert_data(data.get('table_name'), data.get('key'), data)
    #     return up
