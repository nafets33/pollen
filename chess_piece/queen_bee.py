        ...
        if response.status_code == 200:
            ...
            if is_connected and connection_count > 0:
                ...
                for conn in connections:
                    ...
                    logging.info(f"   └─ Connection: {conn_env} {conn}")
...
def get_priceinfo_snapshot(api, ticker, crypto):
    try:
        ...
        print("SNAP CALL ERROR", e, ticker, snap)
...
def handle_broker(broker):
    ...
    if broker in ['auto', 'queens_choice', 'alpaca']:
...
def update_origin_order_cost_basis_current(QUEEN, queen_order_idx, priceinfo, instance=1):
    ...
    if qty_avail > 0:
        ...
        print(f"{instance} update_origin_order_cost_basis_current qty_avail <= 0 orderId: {queen_order_idx}, Qty Avail: {qty_avail}")
...
def queenbee(client_user, prod, queens_chess_piece='queen', server=server, loglevel='warning',
    ...
    def route_queen_order(QUEEN, queen_order, queen_order_idx, order_status, priceinfo, save_order=False):
        ...
        def alpaca_queen_order_state(QUEEN, order_status, queen_order, queen_order_idx, priceinfo, save_order):
            try:
                ...
                print('queen router failed idx:', queen_order.get('client_order_id'), e, print_line_of_error())
    ...
    def update_origin_order_qty_available(QUEEN, run_order_idx, RUNNING_CLOSE_Orders, RUNNING_Orders, save=False):
        try:
            ...
            if queen_order['queen_order_state'] in RUNNING_Orders:
                ...
                QUEEN['queen_orders'].at[run_order_idx, 'qty_available'] = float(queen_order['filled_qty'])
            elif queen_order['queen_order_state'] == 'final':
                pass
            else:
    ...
    def king_knights_requests(QUEEN, STORY_bee, revrec, tm_trig, trigbee, ticker, ticker_time_frame, trading_model, trig_action, trigger_model={}, crypto=False, WT=WT, order_type='buy'):
        s_ = datetime.now(est)
        ...
        def check_revrec(
            ...
            try:
                ...
                if close_order_today or app_request:
                    ...
                    return sell_qty
                if not sell_reasons:
                    # logging.info(f"{ticker_time_frame} SELL REASONS: {sell_reasons}")
                    return 0
                ...
                if sellable <= 0:
                    logging.info(f"CK-REVREC: {ticker_time_frame} NOT Allowed to SELL Min Allocation LONG: {round(current_long)} MIN: {round(min_allocation)} SELLABLE:{sellable}")
                    return 0
                ...
                if adjust_qty != sell_qty:
                    logging.info(f"CK-REVREC:{ticker_time_frame} SELL QTY ADJUSTMENT: {adjust_qty} sell qty: {sell_qty}")
                if adjust_qty <= 0:
                    logging.info(f"CK-REVREC: {ticker_time_frame} NOT Allowed to SELL Adjusted QTY == {adjust_qty} : Sell Qty == {sell_qty}")
                    return 0

                original_qty_available = sell_qty  # This was passed in from run_order['qty_available']
                if crypto:
                    original_qty_available = Decimal(str(sell_qty))

                if adjust_qty > original_qty_available:
                    # Convert to float for display in the log message
                    delta = float(adjust_qty) - float(original_qty_available)
                    print(f"CK-REVREC: {ticker_time_frame} Adjusted qty {adjust_qty} exceeds order's qty_available {original_qty_available}. Capping to available.: delta {delta}")
                    adjust_qty = original_qty_available
            ...
            def waterfall_sellout_chain(client_order_id, 
                ...
                try:
                    ...
                    if sell_qty > 0 and sell_reasons:
                        msg = ("Bishop SAYs SELL:", client_order_id, ticker_time_frame, sell_reasons, current_macd, sell_qty, mm_cost)
                        print(msg)
                        # logging.info(msg)
    ...
    def order_management(BROKER, STORY_bee, QUEEN, QUEEN_KING, api, QUEENsHeart, charlie_bee, mkhrs='closed', ACTIVE_SYMBOLS=[]): 
        ...
        def queen_orders_main(BROKER, QUEEN, STORY_bee, QUEEN_KING, charlie_bee, mkhrs, ACTIVE_SYMBOLS):
            ...
            try: # Order Loop
                ...
                df_active = df_active[df_active['queen_order_state'].isin(active_queen_order_states)].copy()
                qo_active_index = df_active['client_order_id'].to_list()
                ...
                for idex, idx in enumerate(qo_active_index):
                    ...
                    try: 
                        ...
                        # WORKERBEE not working as expected, order_status has updates but lastmod not triggering for queen_order update
                        ...
                        # WORKERBEE always update to handle status issue where run_order not being refreshed with latest order_status ... not sure why ? # WORKERBEE
                        run_order, save_order = update_latest_queen_order_status(
                            QUEEN=QUEEN, 
                            queen_order_idx=idx, 
                            order_status=order_status, 
                            save_order=save_order
                        ...
                        if not run_order:
                            print("ROUTE QUEEN ORDER FAILED for ", idx)
                            continue
                        ...
                        if king_eval_order['bee_sell']:
                            ...
                            if exx.get('executed'):
                                ...
                                run_order_closed, save_order = update_latest_queen_order_status(
                                    QUEEN=QUEEN, 
                                    queen_order_idx=queen_order_idx, 
                                    order_status=order_status, 
                                    save_order=save_order
    ...
    try:
        ...
        db_keys_last_check = datetime.now(est) - timedelta(seconds=61)
        cached_db_keys_df = None
        ...
        while True:
            ...
            if (datetime.now(est) - db_keys_last_check).total_seconds() > 8:
                # Only fetch keys you actually check
                keys_to_check = [QUEEN_KING['key']]  
                results = PollenDatabase.get_specific_keys_timestamps(table_name, keys_to_check, server=server)
                db_keys_df = pd.DataFrame(results, columns=['key', 'timestamp']).set_index('key')
                cached_db_keys_df = db_keys_df
                db_keys_last_check = datetime.now(est)
            else:
                db_keys_df = cached_db_keys_df
            """ The Story of every Knight and their Quest """
            s = datetime.now(est)
            
            #### refresh db QUEEN_KING
            s_time = datetime.now(est)
            if pg_migration:
                qk_lastmod = db_keys_df.at[QUEEN_KING['key'], 'timestamp']
            ...
            """Account Info"""
            QUEEN = refresh_broker_account_portolfio(api, QUEEN)
            acct_info = QUEEN['account_info'] # portfolio = QUEEN['portfolio']
        ...
        send_email(subject="queen crashed", body=json.dumps(log_msg, indent=2, default=str))
...
if __name__ == '__main__':
    ...
    queenbee(client_user, prod, 
             queens_chess_piece=f'queen{crypto}', 
             server=server, 
             loglevel=loglevel, 
             demo=demo,
             )
